...
function randomUUIDv4(): string {
  ...
  if (c && typeof c.getRandomValues === 'function') {
    ...
    // biome-ignore lint/style/noNonNullAssertion: noUncheckedIndexedAccess; index always in bounds
    bytes[6] = (bytes[6]! & 0x0f) | 0x40
    // biome-ignore lint/style/noNonNullAssertion: noUncheckedIndexedAccess; index always in bounds
    bytes[8] = (bytes[8]! & 0x3f) | 0x80
    const hex: string[] = []
    // biome-ignore lint/style/noNonNullAssertion: loop index always within bounds
