...
function makeOpts(strategy = 'post') {
  return {
    ...
    isIntent: (v: unknown): v is { strategy: string; fileId: string } =>
      typeof v === 'object' && v !== null && 'strategy' in v,
...
describe('deserializeSnapshot - NaN-bypass defense', () => {
  ...
  test('NaN progress.uploadedBytes drops progress (item still loads with zero)', () => {
    ...
    const item = state?.items.get('u1') as { progress?: { uploadedBytes: number } } | undefined
