...
export type StrategyCtx<M extends IntentMap, C extends CursorMap<M>, P extends string, R extends UploadResultBase, K extends keyof M & string> = {
  file: File
  intent: M[K]
  signal: AbortSignal
...
export type UploadStrategy<M extends IntentMap, C extends CursorMap<M>, P extends string, R extends UploadResultBase, K extends keyof M & string> = {
  id: K
  resumable: boolean
  start(ctx: StrategyCtx<M, C, P, R, K>): Promise<void>
}

export interface StrategyRegistry<M extends IntentMap, C extends CursorMap<M>, P extends string = string, R extends UploadResultBase = UploadResultBase> {
