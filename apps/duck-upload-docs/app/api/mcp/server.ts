...
const CONTENT_DIR_ENV_VAR = 'DUCK_UPLOAD_DOCS_CONTENT_DIR'
...
export function createMcpServer(): McpServer {
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions: [
        'MCP server for @gentleduck/upload documentation (upload.gentleduck.org).',
        ...
        'Tip: use semantic_search for conceptual queries ("how to handle access control"), search_docs for specific keywords. Use list_docs to browse, then read_doc for details.',
        'Categories: advanced, core, course, guides, installation, integrations, general.',
      ].join(' '),
    },
  )
  ...
  server.tool(
    ...
    {
      slug: z.string().describe('Doc page slug, e.g. "core/roles", "installation/next", "guides/getting-started"'),
      mode: z
        .enum(['full', 'summary'])
        .default('full')
        .describe(
  ...
  server.tool(
    'search_docs',
    'Search documentation by keyword with typo tolerance. Returns matching pages with context snippets, ranked by relevance. Handles misspellings.',
    {
  ...
  server.tool(
    ...
    {
      slug: z.string().describe('Doc page slug, e.g. "core/roles", "installation/next"'),
    },
    async ({ slug }) => {
  ...
  server.tool(
    'semantic_search',
    'Natural language search using TF-IDF vectors and cosine similarity. Better than keyword search for conceptual queries like "how to handle access control" or "role-based permissions". Understands meaning, not just keywords.',
    {
      query: z
        .string()
        .describe(
          'Natural language query, e.g. "how to create role-based access control", "permission policies", "integrating with frameworks"',
        ),
      category: CATEGORY_ENUM.default('all').describe('Narrow search to a category'),
